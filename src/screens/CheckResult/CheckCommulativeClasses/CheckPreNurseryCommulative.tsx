...
const CheckPreNurseryCommulative: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-prenurseryCommulative/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(PreNurseryCommulativeApi);
      console.log(data);
...
export default CheckPreNurseryCommulative;
