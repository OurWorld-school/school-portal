...
const CheckNursery1Commulative: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-nursery1Commulative/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Nursery1CommulativeApi);
      console.log(data);
...
export default CheckNursery1Commulative;
