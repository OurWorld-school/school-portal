...
const CheckBasic5Commulative: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      `https://ourworldintschool.onrender.com/api/basic5Commulative/results/${userId}/${selectedYear}/${selectedTerm}`
    )
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-basic5Commulative/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic5CommulativeApi);
      console.log(data);
...
export default CheckBasic5Commulative;
