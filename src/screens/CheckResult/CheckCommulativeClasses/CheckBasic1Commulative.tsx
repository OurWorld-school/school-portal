...
const CheckBasic1Commulative: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      `https://ourworldintschool.onrender.com/api/basic1Commulative/results/${userId}/${selectedYear}/${selectedTerm}`
    )
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-basic1Commulative/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic1CommulativeApi);
      console.log(data);
      ...
      <div>
        <div className="check-result-flex-div">
          ...
          <div className="select-check-div">
            <div>
              <h2>Commulative Result Checker</h2>
...
export default CheckBasic1Commulative;
