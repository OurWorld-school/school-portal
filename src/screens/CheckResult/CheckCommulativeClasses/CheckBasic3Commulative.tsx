...
const CheckBasic3Commulative: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      `https://ourworldintschool.onrender.com/api/basic3Commulative/results/${userId}/${selectedYear}/${selectedTerm}`
    )
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-basic3Commulative/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic3CommulativeApi);
      console.log(data);
...
export default CheckBasic3Commulative;
