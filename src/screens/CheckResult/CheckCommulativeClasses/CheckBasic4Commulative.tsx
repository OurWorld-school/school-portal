...
const CheckBasic4Commulative: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-basic4Commulative/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic4CommulativeApi);
      console.log(data);
...
export default CheckBasic4Commulative;
