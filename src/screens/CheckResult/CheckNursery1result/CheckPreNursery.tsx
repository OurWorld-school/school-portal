...
const CheckPreNursery: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      `http://localhost:5000/api/prenurseryresult/results/${userId}/${selectedYear}/${selectedTerm}`
    )
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-result-prenursery/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(PreNurseryresultApi);
      console.log(data);
...
export default CheckPreNursery;
