...
const CheckBasic1Result: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      ...
      .then((data) => {
        setApiData(data);
        setLoading(false);
        navigate(`/my-result-basic1/${selectedYear}/${userId}/${selectedTerm}`);
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic1resultApi);
      console.log(data);
...
export default CheckBasic1Result;
