...
const CheckBasic6Result: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      `https://ourworldintschool.onrender.com/api/basic6result/results/${userId}/${selectedYear}/${selectedTerm}`
    )
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-results-basic6/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic6resultApi);
      console.log(data);
...
export default CheckBasic6Result;
