...
const CheckNursery3Result: React.FC<Props> = () => {
  ...
  const fetchApiData = () => {
    ...
    fetch(
      `http://localhost:5000/api/nursery3result/results/${userId}/${selectedYear}/${selectedTerm}`
    )
      ...
      .then((data) => {
        ...
        setLoading(false);
        navigate(
          `/my-result-nursery3/${selectedYear}/${userId}/${selectedTerm}`
        );
        console.log(data);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Nursery3resultApi);
      console.log(data);
...
export default CheckNursery3Result;
