...
const AdminNursery1Result: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    try {
      const fetchData = async () => {
        ...
        const response = await fetch(Nursery1resultApi);
  ...
  const submitHandler = (e: any) => {
    ...
    axios
      .put(UpdatePosiionNursery1resultApi + userId, data, headers)
  ...
  const [classes, setClasses] = React.useState("Nursery-1");
  const handleShowUp = () => {
    setShowUp(true);
  };
  ...
  const lockResultHandler = (e: any) => {
    ...
    axios
      .put(DeActivateNursery1resultApi, data, headers)
              ...
              {apiData?.map((item: any, index: any) => (
                <>
                  <tr>
                    ...
                    <td>
                      {" "}
                      <Link to={`/view-nursery1-student-result/${item?._id}`}>
...
export default AdminNursery1Result;
