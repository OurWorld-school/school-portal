...
export const AdminUserBasic5Commulative = () => {
  let classes = "";
  const { id } = useParams();
  const [viewResult, setViewResult] = useState({});
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await axios.get(Basic5CommulativeApi + id);
      console.log(data);
              ...
              <div>
                ...
                <div className="sheet-div">
                  <div style={{ textTransform: "uppercase" }}>
                    Basic 5 SCHOOL TERMINAL RESULT SHEET
                  </div>
                </div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      {" "}
                      {viewResult?.English?.map((item) => (
                        <div key={item._id}>{item?.totalAverage}</div>
                      ))}
                    </td>
                    <td>
                      {" "}
