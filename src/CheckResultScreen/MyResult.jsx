...
export const MyResult = React.forwardRef((props, ref) => {
  ...
  const { selectedSchool, selectedClass, selectedYear, selectedTerm, userId } =
    useParams();
  const [viewResult, setViewResult] = useState([]);
  console.log("viewresult", viewResult);
  const [ranks, setRanks] = useState([]);
  const [grades, setGrades] = useState([]);
  // const { selectedSchool, selectedClass, userId, selectedTerm, selectedYear } =
  useParams();
  useEffect(() => {
    ...
    fetchPosts();
  }, [selectedSchool, selectedClass, userId, selectedTerm, selectedYear]);
  ...
  React.useEffect(() => {
    const fetchPosts = async () => {
      ...
      setGrades(
        data.filter((item) => item?.schoolName?._id === selectedSchool)
      );
    };
