...
export default function UpdateCutOffMark() {
  const classes = useStyles();
  const navigate = useNavigate();
  const { id } = useParams(); // assume route: /edit-mark/:id
  ...
  // 🧠 Fetch existing cut-off mark to edit
  useEffect(() => {
    if (id) {
      setLoading(true);
      axios
        .get(`${CutOffMarkApi}/${id}`)
        .then((res) => {
          const data = res.data;
          setSchoolName(data.schoolName);
          setName(data.name);
          setCutOffMark(data.cutOffMark);
          setLoading(false);
        })
        .catch((err) => {
          console.error("Error fetching cut-off mark:", err);
          setLoading(false);
        });
    }
  }, [id]);
  ...
  const updateHandler = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.put(`${CutOffMarkApi}/update/${id}`, data, {
        headers,
      });
      setLoading(false);
      ...
      console.error(err);
      setLoading(false);
      toast.error("Error: Unable to update cut-off mark.");
    }
  };
